...
if [[ -z "$CLOUDFRONT_ALIASES" && $STACK_EXISTS == true ]]; then
  CLOUDFRONT_ALIASES=$(read_optional_parameter "CloudFrontAliases" || true)
fi

if [[ -z "$CLOUDFRONT_ACM_CERTIFICATE_ARN" && $STACK_EXISTS == true ]]; then
  CLOUDFRONT_ACM_CERTIFICATE_ARN=$(read_optional_parameter "CloudFrontAcmCertificateArn" || true)
fi

if [[ "$CLOUDFRONT_ALIASES" == "None" ]]; then
  CLOUDFRONT_ALIASES=""
fi

if [[ "$CLOUDFRONT_ACM_CERTIFICATE_ARN" == "None" ]]; then
...
PARAM_OVERRIDES=(
  ...
  "CloudFrontAliases=$CLOUDFRONT_ALIASES"
