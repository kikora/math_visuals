...
- Bekreft i CloudFormation-eksportene for datasstacken (f.eks. `math-visuals-data-RedisEndpointParameterName`) at parameter-/secret-navnene matcher outputene fra `infra/shared-parameters.yaml` før du kjører `deploy-infra.yml`; API-stacken kan da importere både nettverket og Redis-tilkoblingsverdiene uten å provisjonere ny Redis.
